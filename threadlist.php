...
<body>
    ...
    $catid = $_GET['catid'];
    $sql = "SELECT *FROM `teams` WHERE `team_id`='$catid'";
    $result = mysqli_query($conn, $sql);
    if (!$result) {
        die(mysqli_error($conn));
    }
    while ($row = mysqli_fetch_assoc($result)) {
        $cat_title = $row['team_name'];
        $cat_desc = $row['team_description'];
    }
    ...
    <div class="container col-md-8 my-5" id="space">
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if ($login and strtolower($_SESSION['users_name']) == strtolower($user_name)) {
               ...
               <button style="margin: 0px 5px 0px 60px;" class="btn btn-danger" type="button"> <a href="delete.php ? did=' . $catid . ' &  did1=' . $thread_id . '"class = "text-light">Delete</a></button>
